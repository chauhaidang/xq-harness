...
3. Use `rg` or `rg --files` to locate only the files relevant to the task
4. Read the relevant issue, spec, module, or process documentation
...
- [ ] Relevant issues, specs, docs, or PR context are updated when applicable
- [ ] Known blockers and verification gaps are reported
...
- **Scope ambiguity**: re-read the current task, issue, spec, or PRD, then ask the user
